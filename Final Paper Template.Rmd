...
t <- 8
psi_mle <- (n - 2*t) / (n - t)
psi_se <- sqrt(((1-psi)*(2-psi_mle)^2) / n)
...
set.seed(414)
# psi_mle <- 0.3
# data_sets <- rbinom(n = 10000, size = 8, prob = 8/170)
# 
# psi_mle <- function(x){
#   psi <- (1-2*x) / (1-x)
#   return((psi[1] + psi[2] - psi[3])/sum(psi))
# }
# mles <- apply(data_sets, MARGIN= 2, psi_mle)
# sd(mles)
...
The likelihood ratio statistic is calculated by $W = 2\ln\Lambda$, which is equivalent to $2\ln(\frac{L(\hat{\psi}^{mle})}{L(\hat{\psi}^{null})})$. Since $H_0: \psi = 0.3$, we are solving for $2\frac{\ell(\hat{\psi}^{mle})}{\ell(0.3)}$.
