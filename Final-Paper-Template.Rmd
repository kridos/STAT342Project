...
```
We choose these apriori beliefs to analyze the data and sensitivity of the prior. In other words, there are more pessimistic to more optimistic probabilities assigned to the events where the vaccine exceeds the FDA efficacy threshold of 30%, and whether it provides any beneficial value.

Next, we calculate the posterior distributions and Bayesian p-values for the five priors with data model $f(T|\pi) \sim Binom(170, \pi)$. Here, the hypotheses are:
...
Thus, to calculate the Bayesian probabilities: $P(\pi \geq \frac{7}{17}|t) = 1 - P(\pi \leq \frac{7}{17}|t)$ where $t = 8$.
...
For all five priors, the resulting Bayesian P-values are essentially 0. This indicates extremely strong posterior evidence that the vaccine efficacy exceeds the FDA threshold of 30%. The conclusion is therefore robust to the choice of prior, whether it is overly pessimistic or optimistic.
