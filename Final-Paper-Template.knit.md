...
One approach to finding a similar confidence interval to what was reported in the efficacy study is using likelihood inference, which follows the Frequentist framework. Even though the Bayesian framework is used in the study, we thought using the likelihood approach would still help us come to similar conclusions about the data. 

The first method we will use in likelihood inference is maximum likelihood, which identifies the values of the model parameters that makes our data most likely. For maximum likelihood, we need to find a likelihood function for $\psi$ and the maximum likelihood estimator of $\psi_0$. To find our likelihood function $L^{\ast}(\psi)$, we can use theorem 11.1. This states how the likelihood function for a discrete random variable $X$, parameterized by $\theta$, is $L(\theta) = P_{\theta}(X = x_{obs})$, where $x_obs$ is our observed data. Since $T$ follows the binomial distribution, the likelihood function in terms of $\pi$ is $L^{\ast}(\pi)  = \binom{n}{t} (\pi)^{t} \times (1-\pi)^{n-t}$. In this case, $n$ represents the number of people included in the sample and $t$ is the number of people who got infected with COVID. To get $L^{\ast}(\psi)$, we must find $\pi$ in terms of $\psi$ and replace the respective $\pi$ values. To do this, we can use our parameterization $\frac{1-2\pi}{1-\pi} = \psi$:
...
Using $\pi$ in terms of $\psi$, we get the likelihood function: $L^{\ast}(\psi) = \binom{n}{t} (\frac{1 -\psi}{2 -\psi})^{t} \times (1-\frac{1 -\psi}{2 -\psi})^{n-t}$

Now that we have the likelihood function of $\psi$, the next step is to solve for the maximum likelihood estimator (MLE) of $\psi$, $\hat{\psi}^{mle}$. Definition 11.3 explains how the maximum likelihood estimator of the true, unknown parameter value is the value that makes the likelihood function the largest. In order to find this value, we need to find the log-likelihood function of $L^{\ast}(\psi)$, denoted as $\ell(\psi)$. We use $\ell{(\psi)}$ since $\ell{(\psi)}$ is easier to differentiate and has the maximum at the same $\psi$ location as $L^{\ast}(\psi)$.
...
Next we have to to check if $\hat{\psi}^{mle}$ is truly a maximum. To do this, we perform the 2nd derivative test. We check if the 2nd derivative is less than 0:
...
### Significance Test for $\psi_0$

The likelihood ratio statistic is calculated by $W = 2\ln\Lambda$, which is equivalent to $2\ln(\frac{L(\hat{\psi}^{mle})}{L(\hat{\psi}^{null})}) = 2\left[\ln{L(\hat{\psi}^{mle})} - \ln{L(\hat{\psi}^{null})}\right]$.
